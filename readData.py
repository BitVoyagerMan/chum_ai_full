...
with strategy.scope():
    ...
    df = pd.read_csv('Sport_Shoes_1.csv')
    ...
    import csv
    cnt = 0
    filename = "classification_records.csv"
    fields = ['Image_url','chum_product_id', 'classID'] 
    ...
    for everyCategory in groupedData.to_numpy():
        for everyImageUrlArray in everyCategory[0]:
            ...
            print(array[0], everyCategory[1][cnt])
            
            print(classify_image(array[0]))
            csvwriter.writerows([[array[0], everyCategory[1][cnt], classify_image(array[0])]])
            cnt += 1
        break
